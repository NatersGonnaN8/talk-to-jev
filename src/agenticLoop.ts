...
const AWARENESS =
  "Jev cannot invent answers that were not given. Choice = listed options only (mill numbers + those descriptions). Noul = P(true). Score = a legend level. If you need a new option, call read_jev_workshop or read_jev_questions then set_jev_questions then ask_jev again. Do not invent a new random state.";
...
export function agenticLoopFirstPrompt(total: number) {
  ...
  return `Agentic loop · ${count}. Use the current Jev’s State and current questions. Do not invent a new random state. Call ask_jev. If you change questions, call read_jev_workshop or read_jev_questions first. Do not paste JSON in the chat. Do not wait for me.`;
}
...
export function agenticLoopContinuePrompt(
  ...
  const instruction = last
    ...
    : `Agent turn ${t} of ${n}. Continue the agentic loop on the current pane. You may update questions (read_jev_workshop or read_jev_questions then set_jev_questions) then ask_jev if you need a new snap. Do not invent a new random state. Reason about the probabilities. Do not wait for the operator. ${AWARENESS}`;
