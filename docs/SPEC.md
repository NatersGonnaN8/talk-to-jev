...
- Question editor: add / remove questions
  ...
  - **Choice criteria sent to Jev (2026-09-20).** TypeSafe Choice is a map of **option name → description**; both go to the model. Our names are the mill numbers. The Decisions payload must be positional keys → **description text**: `{ "1": "papaya", "2": "banana" }`, never `{ "1": "1" }` (description = key) and never empty values when the card has text. Score still sends ordered legend strings (array). Do not bring back the option-key input. Do not stop Space → `_` on description typing.
