...
|---|---|---|---|
| **LLM** | `deepseek/deepseek-v4-flash` | `POST /api/v1/chat/completions` | Talk. Draft. **Tools** write Jev’s case and Jev’s Questions. **Random case** runs an autonomous LLM↔Jev loop. Explain answers. Never a JSON dump as the product. |
...
| **Feed Jev → LLM** | Reverse wire. Build a user-visible summary of typed answers (choice / noul / score / confidence) and **immediately stream** `/api/llm` on the **same path as Send** (SSE: delta / thought / tool / done). Show the You bubble, then mill thinking / the streamed assistant reply. Do **not** park the note for a second Send click. Do **not** insert a canned assistant one-liner. Disabled when there are no Jev answers yet (do not fake a send). Does **not** go through LLM tools. Nater (2026-09-20): pass immediately to the LLM, same path as hitting Send. **Random case** later turns use this same send-now path so Jev’s typed answers round-trip into the next LLM turn. |
| **Random case** | LLM pane-head **mill slot**, **left of Propose Jev questions**. Label **Random case** (mixed case, Public Sans like `.nav-btn` — not ALL CAPS, not a stamp). Click opens a mill popover: **How many turns do you want to do?** with options **3** through **10**. Opaque manila, fully on-screen, flip above/below, high z-index. **Confirm** starts the run; **Cancel** (or Escape / outside click) aborts — no run. Then a full autonomous run of that many **turns** (definition below). The operator does not type between turns. Unlock the Workshop when the loop finishes or errors. Does **not** load a Preset Cases snap. Nater (2026-09-20). |
...
**Empty Workshop (first-open with no active thread, and New Case):** no sample id. Jev’s case textarea empty. One blank-id noul question (same as **Add question**). No Jev answers. Empty LLM thread. Weather row hidden. **Preset Cases** none selected. **Invoice exception** stays in the catalog — pick it from **Preset Cases**, Use Cases, or `?case=invoice`. It is not the empty session. Nater (2026-09-20), on **New Case**: “somehow this brings it to preset case about the invoices, fix please.” See §12.

### 5.1 Random case — turn definition and loop

A **turn** is one `/api/llm` SSE session: one You bubble plus one assistant stream (thoughts / tool cards / prose). During a turn the LLM may call `set_jev_case`, `set_jev_questions`, and `ask_jev`. **`ask_jev` actually POSTs `/api/jev`** (same Decisions path as the **Ask Jev** button). Typed answers apply to the Jev pane immediately. They **round-trip into the next turn**: the next `/api/llm` body includes `jevAnswers`, and the user message is the **Feed Jev send-now** note (typed summary streamed on the same path as Send — not a canned assistant one-liner). The operator does not type between turns.

Chosen count **N** is **3–10**. That many turns run:

1. **Turn 1.** LLM invents a **short** imaginary scenario (just enough facts to judge) → `set_jev_case` fills Jev’s case. Then **3–5** atomic questions, **at least one noul, one score, and one choice** → `set_jev_questions` (real **snake_case** ids; choice keys become mill numbers `"1"` / `"2"` / …; **descriptions are the values**). Then `ask_jev`. Mode on this request: `random-case`.
2. **Turns 2 … N−1.** LLM may update case or questions, `ask_jev` again if it needs a new snap, then reason about the probabilities. Each of these starts with the Feed Jev send-now note from the latest answers (mode `chat`).
...
- Secondary: **Random case** (mill slot, **left of** Propose) then **Propose Jev questions**
- After Jev has answered: **Feed Jev to LLM**. Click builds the Jev-answers user message and **immediately POSTs** `/api/llm` (same `sendLlm` / SSE path as **Send**). The composer may stay empty; **Send** stays disabled until they type. Show the You bubble, then mill thinking / streamed assistant reply. Do **not** wait for a second click. Do **not** insert a canned “Got Jev’s typed answers…” assistant line. If there are no Jev answers yet, keep the button disabled (do not fake a send). **Random case** later turns reuse this send-now path.
- **Random case** mill popover: **How many turns do you want to do?** Options **3–10**. Confirm starts; Cancel aborts. No ALL CAPS. Opaque, flip, fully on-screen, high z-index. See §5.1.
...
5. **Propose Jev questions** (tools fill the q-cards) / **Feed Jev to LLM** (sends typed answers to the LLM immediately) / **Random case** (mill slot, turn picker) if those buttons exist.
...
- It has **tools**. Use them. Do **not** paste case JSON or a questions map into the chat as the product.
  ...
  3. **`ask_jev`** — only if case + questions are **clean** (at least one real-id question, no blank ids in the payload). Calls existing `/api/jev` (OpenRouter Decisions). If not clean: tools 1–2 only, and tell the operator to click **Ask Jev** (except **random-case** mode: keep using tools until `ask_jev` succeeds or rounds run out). Typed answers in the tool result **and** on the SSE event must round-trip: the UI applies them, and the **next** `/api/llm` turn sends `jevAnswers` plus the Feed Jev send-now user note.
...
**`ask_jev`** — `{}`. Uses the working case + working questions from this request (after any `set_*` in the same loop). Choice keys in that call are the positional numbers. If not clean: `{ ok: false }` to the model; do not call Decisions; the operator clicks **Ask Jev** (random-case mode keeps trying tools). On success, the tool payload includes `answers` so the model can reason in-loop, and the SSE event carries those answers so the **next client turn** can send `jevAnswers` + the Feed Jev note.
...
ChatThread:
  ...
  samplePresetId      // §12 preset id, or null on the empty Workshop (New Case / first-open)
```
...
**Empty Workshop:** first-open (no active thread) and **New Case** load **no** preset. Invoice exception (`invoice`) is catalog item 1 — pick it from **Preset Cases**, Use Cases, or `?case=invoice`. It is **not** selected on the empty Workshop. Jacket remains available in the menu and as a Use Cases card.
...
20. First-open Workshop (empty history): **Preset Cases** has **none selected**; weather row is **hidden**; Jev’s case is blank. Jacket from the menu shows the weather row; switching back to a business preset hides it again
...
53. **Random case:** **New Case** (empty Workshop) → **Random case** (mill slot, left of Propose, mixed-case Public Sans) → pick **3** turns → **Confirm**. Jev’s case fills (`set_jev_case`). Q-cards show mixed types (at least one noul, one score, one choice; mill numbers; snake_case ids). `ask_jev` POSTs `/api/jev`. Answers return; a later `/api/llm` SSE streams analysis on the Feed Jev send-now path (not a canned one-liner). Turns 2–3 continue without the operator typing. Mill shows **Turn k of 3**. Workshop **unlocks** when done. Popover **Cancel** does not start a run. Tip/popover stays fully on-screen, opaque, no ALL CAPS.
