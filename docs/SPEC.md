...
| **Feed Jev → LLM** | Reverse wire. **Chrome:** **Jev’s Questions** pane-head (`article.pane.jev`), immediately **left of Ask Jev**, same row as the title. **Not** on the LLM mill. Build a user-visible summary of typed answers (choice / noul / score / confidence) and **immediately stream** `/api/llm` on the **same path as Send** (SSE: delta / thought / tool / done). Show the You bubble, then mill thinking / the streamed assistant reply. Do **not** park the note for a second Send click. Do **not** insert a canned assistant one-liner. Disabled when there are no Jev answers yet (do not fake a send). FlipTip + `title`: **Ask Jev first — nothing to feed.** Does **not** go through LLM tools. Nater (2026-09-20): pass immediately to the LLM, same path as hitting Send. Same day: selected **Feed Jev to LLM** on the LLM mill and said **move this to Jev’s side**. **Random state** analysis and **Agentic loop** later turns use this same send-now path so Jev’s typed answers round-trip into the next LLM turn. |
