...
|---|---|---|---|
| **LLM** | `deepseek/deepseek-v4-flash` | `POST /api/v1/chat/completions` | Talk. Draft. **Tools** write Jev’s State and Jev’s Questions. **Random state** runs an autonomous LLM↔Jev loop. Explain answers. Never a JSON dump as the product. |
...
| **Feed Jev → LLM** | Reverse wire. Build a user-visible summary of typed answers (choice / noul / score / confidence) and **immediately stream** `/api/llm` on the **same path as Send** (SSE: delta / thought / tool / done). Show the You bubble, then mill thinking / the streamed assistant reply. Do **not** park the note for a second Send click. Do **not** insert a canned assistant one-liner. Disabled when there are no Jev answers yet (do not fake a send). Does **not** go through LLM tools. Nater (2026-09-20): pass immediately to the LLM, same path as hitting Send. **Random state** later turns use this same send-now path so Jev’s typed answers round-trip into the next LLM turn. |
| **Random state** | LLM pane-head **mill slot**, **left of Propose Jev questions**. Label **Random state** (mixed case, Public Sans like `.nav-btn` — not ALL CAPS, not a stamp). Click opens a mill popover: **How many turns do you want to do?** with options **3** through **10**. Opaque manila, fully on-screen, flip above/below, high z-index. **Confirm** starts the run; **Cancel** (or Escape / outside click) aborts — no run. Then a full autonomous run of that many **turns** (definition below). The operator does not type between turns. Unlock the Workshop when the loop finishes or errors. Does **not** load a Preset States snap. Nater (2026-09-20). |
...
### 5.1 Random state — turn definition and loop
...
2. **Turns 2 … N−1.** LLM may update state or questions, `ask_jev` again if it needs a new snap, then reason about the probabilities. Each of these starts with the Feed Jev send-now note from the latest answers (mode `chat`).
...
The mill slot shows **Random state** when idle and mill thinking (**Turn k of N**) while the run is in flight. Unlock when the loop finishes or errors.
...
- Full Convert to Markdown UI — not a half-pane on Workshop, not a button on Jev’s State. Page title **Convert to Markdown** (`h1.pane-title`) and pane heads **Files** / **Markdown** (`h2.pane-title`) match **Jev’s Questions** size. Kicker `.eyebrow` (**In this browser**) stays smaller.
...
- **Feed Jev to LLM** is the reverse wire — a user note in the thread that **immediately streams** `/api/llm` (same path as Send). Not a tool. Not a canned assistant one-liner. Random state turns after the first use this path so answers reach the LLM.
...
**`ask_jev`** — `{}`. Uses the working state + working questions from this request (after any `set_*` in the same loop). Choice keys in that call are the positional numbers. If not clean: `{ ok: false }` to the model; do not call Decisions; the operator clicks **Ask Jev** (random-case mode keeps trying tools). On success, the tool payload includes `answers` so the model can reason in-loop, and the SSE event carries those answers so the **next client turn** can send `jevAnswers` + the Feed Jev note.
