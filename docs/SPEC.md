...
A **turn** is one `/api/llm` SSE session: one You bubble plus one assistant stream (thoughts / tool cards / prose). During chat or **Agentic loop**, the LLM may call `set_jev_case`, `set_jev_questions`, and `ask_jev`. **`ask_jev` actually POSTs `/api/jev`** (same Decisions path as the **Ask Jev** button). Typed answers apply to the Jev pane immediately.
...
**Visibility (required, not optional)** — Nater (2026-09-20, first): “All of the scroll buttons need to have their disappear logic not tied to my use of that certain window or tab or modal or whatever, but connected to the actual space that the scrollbar takes up itself, okay?” Then: **“scroll bars i mean”** — mill overlay `.mill-bar` (track + thumb + `.mill-bar-arrow` buttons), not other chrome.

- Default: thumb + arrows **opacity 0** (fully gone). Track stays invisible.
- **Hold opaque** only while the pointer is over **that scrollbar’s own hit area** — the overlay strip `.mill-bar` (arrows + track + thumb). **Not** the whole `.thread` / `.jev-scroll` / page / tab / Inspector / Settings card / mill pane.
- Hovering mill **content** (chat text, Jev questions, Settings keys, Docs prose) must **not** pin the bars visible.
- **Leave that strip** → stay **fully opaque for 1.0s**, then **fade to opacity 0 over the next 1.0s**. No other timings. Do **not** restart those timers because the operator is still “in the mill pane,” Inspector/modal is open, or they switched tab. Leaving the mill pane for Jev’s Questions / Inspector / another tab is **not** the fade trigger; leaving the **bar** is.
- **Show** on scroll (wheel / trackpad / drag / keyboard-driven `scroll`) of **that scroller** is still OK (briefly `is-on`), then the same leave-the-**bar** timers apply — not leave-the-window / leave-the-pane.
- Hit testing: `pointerenter` / `pointerleave` on **that** `.mill-bar`. If `relatedTarget` is still inside the same `.mill-bar`, do not start fade (moving arrow → track → thumb is still on the strip). Pointer-events on `.mill-bar` when visible / `is-on` and during the fade so the strip can receive those events; at rest (`opacity 0`, not fading) pointer-events **none** so the invisible strip does not steal clicks. Do **not** use `document` / `window` / `visibilitychange` / tab as the hide host.
...
57. Overflow scrollers (Jev’s State textarea, `.thread`, `.jev-scroll`, Docs `.doc-list` / Nice view, Convert, History, Inspector JSON, and the rest in §8.1) show **no white native track**. Hover mill **content** (chat / questions / keys): bars are **not** pinned. Hover that scroller’s `.mill-bar` strip (thumb/arrows): mill-green thumb + arrows fully opaque (`is-on`, pointer-events allow it). Leave **the strip** (even while still in the LLM pane): stay opaque **1.0s**, then fade **1.0s**. Opening Inspector or switching Settings mid-fade must **not** reset that bar’s timers. Idle: opacity 0. No reserved white gutter. Native `resize` still `none`.
