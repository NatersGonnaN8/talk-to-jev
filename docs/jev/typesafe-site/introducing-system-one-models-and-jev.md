...
Nuance- For people with early access to TypeSafe, here is the actual query.
...
Nuance- This is where the claims of 193.6x faster, 444.6x cheaper on our home page comes from, and we expect that these are on the higher end of real world gains.
...
Nuance- The numbers for LLMs are from OpenRouter i.e., there almost certainly is bias here: more complex queries might be routed to better models.
...
Nuance- The demo is on structured state as a data structure with text, not on images (yet…)
...
Nuance- As far as we know, it was completely random that both the 2nd and 3rd challenges started with “Rubber Duck.” The author only noticed when the team pointed it out.
