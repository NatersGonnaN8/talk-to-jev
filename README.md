...
|---|---|---|
| **Workshop** | `/` | **Jev’s case** + LLM pane + Jev pane. First-open is an **empty** Workshop. **New Case** starts another empty Workshop (does **not** load Invoice). **Preset Cases** (9 business + Jacket), **History** on that row. **Load weather** on Jacket only. **Ask Jev**, **Propose Jev questions**, **Feed Jev to LLM**. Drop `.md` into Jev’s case; other files open the **Convert** tab. |
